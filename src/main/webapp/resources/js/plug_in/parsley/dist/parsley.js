  ...
  Validator.prototype = {
    ...
    */
    validate: function ( objectOrString, AssertsOrConstraintOrGroup, group ) {
      ...
      // com.bigDragon.common.regular object validation
    ...
    */
    ...
    Length: function ( boundaries ) {
      ...
      this.validate = function ( value ) {
        ...
        //如果不满足指定的长度
        if ( 'undefined' !== typeof this.min && this.min === this.max && this.getLength(value) !== this.min )
          throw new Violation( this, value, { min: this.min, max: this.max } );
        //如果长度超过最大长度
        if ( 'undefined' !== typeof this.max && this.getLength(value) > this.max )
          throw new Violation( this, value, { max: this.max } );
        //如果长度小于最小长度
        if ( 'undefined' !== typeof this.min && this.getLength(value) < this.min )
          throw new Violation( this, value, { min: this.min } );
        return true;
      };
      return this;
    },
    //返回兼容uft-8中文字符的字符串长度，一个中文占2个字节
    getLength: function(value){
    	...
    	len = value.length;
	 	if( !value ) return 0;
	 	for( var i=0; i<len; i++ ){
	 		byteLen += value.charCodeAt(i) > 255 ? 2 : 1;
	 	}
    	return byteLen;
    },
  ...
  */
    ...
    validators: {
      ...
      type: function (type) {
        ...
        switch (type) {
          ...
          break;
          case 'phone':
            assert = new Validator.Assert().Regexp('(\\d{3,4}-)?\\d{7,8}$', 'i');
          break;
          case 'mobilephone':
             assert = new Validator.Assert().Regexp('(\\+?86)?(13[0-9]\\d{8}|14[57]\\d{8}|15[012356789]\\d{8}|17[3678]\\d{8}|170[059]\\d{7}|18[0-9]\\d{8}|19[0-9]\\d{8})$', 'i');
          break;
          default:
  ...
  ParsleyUI.prototype = {
    ...
    addError: function (fieldInstance, name, message, assert, doNotUpdateClass) {
      ...
      fieldInstance._ui.$errorClassHandler.tooltip('destroy');
      fieldInstance._ui.$errorClassHandler.attr('data-original-title', message || this._getErrorMessage(fieldInstance, assert));
      var tip = fieldInstance._ui.$errorClassHandler.tooltip({
  		title: message || this._getErrorMessage(fieldInstance, assert),
  		placement: 'top',
  		trigger: 'focus',
  		container: 'body',
  		html: true
    ...
    setupForm: function (formInstance) {
      formInstance.$element.on('click.Parsley', 'input[type="submit"]', $.proxy(formInstance.onSubmitValidate, formInstance));
      formInstance.$element.on('submit.Parsley', false, $.proxy(formInstance.onSubmitValidate, formInstance));
   //   formInstance.$element[button].on('click.Parsley', false, $.proxy(formInstance.onSubmitValidate, formInstance));
  ...
  ParsleyForm.prototype = {
    ...
    isValid: function (group, force) {
      ...
      for (var i = 0; i < this.fields.length; i++) {
    	//如果是隐藏元素，则跳过不验
      	if($(this.fields[i].$element[0]).is(":hidden")) continue;
        // do not validate a field if not the same as given validation group
        if (group && group !== this.fields[i].options.group)
          continue;
    ...
    _bindFields: function () {
      ...
      this.$element.find(this.options.inputs).each(function () {
        ...
        if (('ParsleyField' === fieldInstance.__class__ || 'ParsleyFieldMultiple' === fieldInstance.__class__) && !fieldInstance.$element.is(fieldInstance.options.excluded))
          //if(fieldInstance.$element.parents().hasClass("hidden")) return false;
  ...
  ParsleyField.prototype = {
    ...
    validate: function (force) {
      //if(this.$element.parents().hasClass("hidden")) return false;
    ...
    */
    ...
    _bindHtml5Constraints: function () {
      ...
      else if (new RegExp(type, 'i').test('email url range phone mobilephone'))   
...
// Define then the messages
    var promotLanguageCN = {
        defaultMessage: "输入的值不正确",
        type: {
    ...
    var promotLanguageEN = {
        defaultMessage: "The value entered is incorrect",
        type: {
            ...
            alphanum:     "Please enter letters or numbers",
            phone:       "Please enter the correct phone number",
            mobilephone:     "Please enter the correct phone number"
        },
        notblank:       "please input the value",
        required:       'required',
        pattern:        "The format is incorrect",
        min:            "Please enter a value greater than or equal to"+" %s ",
        max:           "Please enter less than or equal to"+" %s ",
        range:          "The input value should be"+ "%s "+ "to" +" %s " +"between",
        minlength:      "Please enter at least"+" %s "+ "Characters",
        maxlength:      "Please enter up to"+" %s "+"Characters",
        length:         "Character length should be"+" %s "+ "to"+" %s "+"between",
        mincheck:       "Please choose at least"+" %s "+"Options",
        maxcheck:       "Please choose not to exceed "+" %s "+"Options",
        check:          "please choose "+" %s " +"to"+" %s "+ "Options",
        equalto:        "The input value is different"
    }
    window.ParsleyConfig.i18n.en = $.extend(window.ParsleyConfig.i18n.en || {}, promotLanguageCN);
    if(langCode==="1"){
        window.ParsleyConfig.i18n.en = $.extend(window.ParsleyConfig.i18n.en || {}, promotLanguageEN);
    }
