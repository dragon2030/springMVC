...
public class CustomerForQuery {
	...
	public void testQuery1() {
		...
		try {
			...
			/**
			 * 执行,并返回结果集
			 * 执行PreparedStatement查询将返回一个结果集
			 * 此结果集类似迭代器
			 * resultSet.next()判断结果集的下一条是否有数据，如果有数据返回true,并指针下移；如果返回false,指针不会下移。
			 * 等于迭代器中的hasNext()和next()方法的合并
			 */
...
//			Object[] data = new Object[]{id,name,email,birth};
	 ...
	 * 	应用了两个技术进行了优化  结果集元数据ResultSetMetaData 和 java反射
	 * @throws Exception
	 * @date 上午10:23:40
	 */
